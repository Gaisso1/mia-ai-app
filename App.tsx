...
const App: React.FC = () => {
  ...
  // Persistence for Rides
  const [rides, setRides] = useState<Ride[]>([]);

  // Check for stored user and rides on load
  useEffect(() => {
    ...
    const savedRides = localStorage.getItem('crew_rides_db');
    if (savedRides) {
  ...
  // Sync Rides to LocalStorage whenever they change
  useEffect(() => {
    if (rides.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-24 md:pb-20">
      {/* Header */}
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveTab('active')}>
            ...
            <div className="flex flex-col">
              <h1 className="text-xl md:text-2xl font-bold tracking-tighter brand-font leading-none">
                CREW
              </h1>
          ...
          <nav className="hidden lg:flex items-center space-x-8 text-sm font-bold uppercase tracking-widest text-slate-400">
            <button 
              ...
              className={`${activeTab === 'active' ? 'text-orange-500 border-b-2 border-orange-500' : 'hover:text-white'} transition-all pb-1`}
            >
              Giri Attivi
            </button>
            <button 
              onClick={() => setActiveTab('my-rides')}
              className={`${activeTab === 'my-rides' ? 'text-orange-500 border-b-2 border-orange-500' : 'hover:text-white'} transition-all pb-1`}
            >
              I Miei Giri
            </button>
            <button 
              onClick={() => setActiveTab('community')}
              className={`${activeTab === 'community' ? 'text-orange-500 border-b-2 border-orange-500' : 'hover:text-white'} transition-all pb-1`}
            >
              Community
            </button>
          </nav>
        ...
        {activeTab === 'community' ? (
          <section className="animate-in fade-in duration-500">
            <div className="mb-10 text-center md:text-left">
              <h2 className="text-4xl font-bold tracking-tight text-white mb-2 italic brand-font">La Community</h2>
              <p className="text-slate-500">I biker che rendono <span className="text-orange-500">CREW</span> leggendaria.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {allCommunityMembers.map(member => (
                <div key={member.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 hover:border-orange-500/50 transition-all group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <aside className="lg:col-span-3 space-y-6">
              ...
              <button 
                ...
                className="w-full py-4 bg-white hover:bg-slate-200 text-slate-900 font-bold rounded-2xl flex items-center justify-center space-x-2 transition-all shadow-xl hover:shadow-white/10 group"
              >
            ...
            <section className="lg:col-span-9">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <div className="lg:hidden w-full">
                  <div className="flex space-x-2 overflow-x-auto pb-2 no-scrollbar">
                    <button 
                      onClick={() => setGeoFilter({region: 'Tutte', province: 'Tutte', location: 'Tutte'})}
                      className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-full text-xs font-bold whitespace-nowrap"
                    >
                      Reset Filtri
                    </button>
                    {geoFilter.region !== 'Tutte' && (
                       <span className="px-4 py-2 bg-orange-500 rounded-full text-xs font-bold text-white whitespace-nowrap">
                        {geoFilter.region}
                       </span>
                    )}
                  </div>
              ...
              {filteredRides.length === 0 ? (
                <div className="bg-slate-900/50 rounded-3xl p-12 md:p-20 text-center border-2 border-dashed border-slate-800">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-600">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 px-6 py-3 flex justify-between items-center shadow-2xl">
        ...
        <button onClick={() => setIsAIModalOpen(true)} className="flex flex-col items-center -mt-8">
          <div className="w-14 h-14 bg-gradient-to-br from-orange-500 to-red-600 rounded-full shadow-2xl flex items-center justify-center border-4 border-slate-950">
          ...
          <span className="text-[10px] font-bold uppercase mt-1 text-slate-500">AI Assist</span>
        </button>
        ...
        <button 
          onClick={() => {
            if(confirm("Vuoi resettare il profilo?")) {
              localStorage.clear();
      ...
      <button 
        ...
        className="hidden lg:flex fixed bottom-6 right-6 w-16 h-16 bg-gradient-to-br from-orange-500 to-red-600 rounded-full shadow-2xl items-center justify-center hover:scale-110 transition-transform active:scale-95 z-40 group"
      >
        <svg className="w-9 h-9 text-white animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
        <span className="absolute right-full mr-4 bg-slate-900 text-white text-xs px-3 py-2 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity border border-slate-700 pointer-events-none">
